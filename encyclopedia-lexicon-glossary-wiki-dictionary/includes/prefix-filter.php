...
abstract class PrefixFilter
{
    ...
    public static function getPrefixes(string $prefix = '', ?WP_Term $taxonomy_term = null): array
    {
        ...
        if ($taxonomy_term) {
            $arr_filter = $wpdb->get_results($wpdb->prepare(
                'SELECT
                    LOWER(SUBSTRING(posts.post_title,1,%d)) prefix,
                    COUNT(ID) items
                FROM
                    %i as posts,
                ...
                WHERE
                    ...
                    posts.post_title LIKE %s AND
                    term_relationships.object_id = posts.id AND
                    term_relationships.term_taxonomy_id = %d
                GROUP BY prefix
                ORDER BY prefix ASC',
                $prefix_length,
                $wpdb->posts,
                ...
                $prefix . '%',
                $taxonomy_term->term_taxonomy_id
            ));
        } else {
            $arr_filter = $wpdb->get_results($wpdb->prepare(
                'SELECT
